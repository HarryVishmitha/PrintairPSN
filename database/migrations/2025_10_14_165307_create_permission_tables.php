...
return new class extends Migration
{
    ...
    public function up(): void
    {
        ...
        Schema::create($tableNames['roles'], static function (Blueprint $table) use ($teams, $columnNames, $teamForeignKey) {
            // $table->engine('InnoDB');
            $table->bigIncrements('id'); // role id
            if ($teams || config('permission.testing')) { // permission.testing is a fix for sqlite testing
                $table->foreignId($teamForeignKey)->nullable()->constrained()->cascadeOnDelete();
            }
            $table->string('name');       // For MyISAM use string('name', 225); // (or 166 for InnoDB with Redundant/Compact row format)
            $table->string('guard_name'); // For MyISAM use string('guard_name', 25);
            $table->timestamps();
        ...
        Schema::create($tableNames['model_has_permissions'], static function (Blueprint $table) use ($tableNames, $columnNames, $pivotPermission, $teams, $teamForeignKey) {
            ...
            $table->foreign($pivotPermission)
                ...
                ->onDelete('cascade');
            if ($teams) {
                $table->foreignId($teamForeignKey)->constrained()->cascadeOnDelete();
